...
function loading(param) {
    ...
    if (typeof param != 'boolean') {
        param = param || '正在提交...';
        var html =
            '<div class="loading-bg-wrap"><div class="loading-bg">' +
...
(function ($) {
    ...
    function creatHtml() {
        ...
        var html =
            ...
            '<div class="ms-toggle-pdf">' +
            text +
            '</div>' +
            '<div class="ms-clear-seal">恢复原图</div>' +
            '<div class="ms-seal-clip" style=" display: none; ">印章:全</div>' +
            '<div class="ms-to-top" style=" display: none; ">回到顶部</div>' +
            '<div class="ms-page-box" style="' +
            showPage +
            '">' +
            '<p></p>' +
            '<p></p>' +
            ...
            '<div class="ms-jump-number" style=" display: none; ">跳到<input maxlength="2">页</div>' +
            '<div class="ms-jump-btn" style=" display: none; ">确定跳转</div>' +
            '</div>' +
            '</div>' +
            '</div>' +
    ...
    function bindFucFile() {
        // 绑定文件上传操作事件
        t.el.change(function () {
            // 监听文件变化
            updataFuc(this.files);
        });
        t.drapBox.on('drop', function (e) {
            // 绑定拖拽事件
            e.stopPropagation();
            e.preventDefault();
            updataFuc(e.originalEvent.dataTransfer.files);
        });
        t.drapBox.on('dragover', function (e) {
            // 绑定拖拽事件
            e.stopPropagation();
            e.preventDefault();
        });
        t.mainBox.find('.ms-select-btn').click(function () {
            // 自定义选择文件
            t.el.click();
        });
        t.mainBox.find('.ms-tranform-btn').click(function () {
            // 转pdf按钮
            postPdf();
        });
        t.mainBox.find('.ms-clear-file').click(function () {
            //清空文件
            updataFuc(false);
    ...
    function bindMenuRight() {
        ...
        t.mainBox.find('.ms-add-seal').click(function () {
            // 自定义盖章
        ...
        t.mainBox.find('.ms-seal-list').on('click', 'ul li', function () {
            // 盖章事件
            var index = $(this).index();
            var png = JSON.parse(localStorage.getItem(localStorage.key(index))).png;
            sealOn(png);
        });
        t.mainBox.find('.ms-seal-list').on('click', 'ul li span', function () {
            // 删除单个印章
        ...
        t.mainBox.find('.ms-seal-list').on('click', '.ms-delete-seal', function () {
            // 删除印章列表
            localStorage.clear();
            updataSeal();
            msg('印章列表清空完毕');
    ...
    function bindMenuFixed() {
        //绑定悬浮列表事件
        t.mainBox.find('.ms-clear-seal').click(function () {
            // 清空印章，重新渲染pdf/图片
            if (t.pdf) {
                showPdf(t.pdf);
            } else if (t.pic) {
        ...
        t.mainBox.find('.ms-fixed-right .ms-toggle-pdf').click(function () {
            // 切换逐页/长图
        ...
        t.mainBox.find('.ms-seal-list .ms-download-btn-single').click(async function () {
            // 下载单张图片
            loading('图片转换中...');
            await sleep(0);
            downLoadSingle();
        ...
        t.mainBox.find('.ms-seal-list .ms-download-btn-long').click(async function () {
            // 下载长图
            loading('图片转换中...');
            await sleep(0);
            downLoadLong();
        ...
        t.mainBox.find('.ms-fixed-right .ms-page-toggle .ms-prev').click(function () {
            // 上一页
            if (t.curPage == 1) {
                msg('已经是第一页了');
                return;
        ...
        t.mainBox.find('.ms-fixed-right .ms-page-toggle .ms-next').click(function () {
            // 下一页
        ...
        t.mainBox.find('.ms-fixed-right .ms-to-top').click(function () {
            //回到顶部
            $('html,body').animate(
                {
                    scrollTop: 0,
        ...
        t.mainBox.find('.ms-fixed-right .ms-jump-btn').click(function () {
            //页面跳转
    ...
    function pageChange() {
        ...
        t.mainBox
            ...
            .show();
        t.mainBox
            .find('.ms-fixed-right .ms-page-box p')
            .eq(1)
            .text('当前页 ' + t.curPage);
    }
    function sealOn(png) {
